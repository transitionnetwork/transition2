...
// Get the profile's user account, see if we can contact them and build a nice button if so
$author = user_load($node->uid);
$contact_link = false;
if ($author->contact) {
  $contact_link = l(
    '<span>' . t('Contact Me') . '<span>',
    'user/'. $author->uid .'/contact',
    array(
      'attributes' => array(
...
// JK NOTE: user profiles use a location field rather than the node location module...
//    SO need to get different location variable in this case
$location = $field_user_location[0];
...
if ($location['province_name']) $location_block .= '<span class="region">' . $location['province_name'] . '</span>, ';
...
if (!$teaser) {
  if ((floatval($location['latitude']) == 0) && (floatval($location['longitude']) == 0)) { // no
    ...
    // for users, need to make the location a little less accurate...
    $latitude = floatval($location['latitude']);
    $longitude = floatval($location['longitude']);
    $tweak = (($node->nid % 500) - 250)/100000;
    $latitude = $latitude + $tweak + rand(-1, 1)/1000;
    $longitude = $longitude - $tweak + rand(-1, 1)/1000;
    $location_block .= gmap_simple_map($latitude, $longitude, '', '<h4>' . $title . '</h4>', 6, '250px', '200px');
    $location_block .= '<small>' . t("Marker made less accurate for better privacy") . '</small>';
  }
}
$location_block .= '</div>'
?>
